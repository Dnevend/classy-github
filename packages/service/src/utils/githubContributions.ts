...
async function fetchDataForYear(url: string, year: string, format?: "nested") {
  ...
  const contribText = $(".js-yearly-contributions h2")
    ...
    .trim()
    .match(/^([0-9,]+)\s/);
  let contribCount: number | string;
  ...
  return {
    ...
    contributions: (() => {
      const parseDay = (day: string, index: number) => {
        const $day = $(day);
        const date = $day
          .attr("data-date")!
          .split("-")
          .map((d) => parseInt(d, 10));
        const color = COLOR_MAP[$day.attr("data-level") as never];
        const value = {
      ...
      if (format !== "nested") {
        return $days
          .get()
          .map((day, index) => parseDay(day as unknown as string, index).value);
      }

      return $days.get().reduce((o, day, index) => {
        const { date, value } = parseDay(day as unknown as string, index);
        const [y, m, d] = date;
        if (!o[y]) o[y] = {};
        if (!o[y][m]) o[y][m] = {};
        o[y][m][d] = value;
